...
export default function MonthView({ currency, month, onMonthChange }: {
  ...
  return (
    <div className="space-y-4">
        ...
        <dl className="mt-3 space-y-1 text-sm text-ink/70">
          {[
            ...
            <div key={label as string} className="flex justify-between gap-4">
              <dt className={String(label).startsWith('=') ? 'font-semibold text-ink' : ''}>{label}</dt>
              <dd className={String(label).startsWith('=') ? 'font-semibold text-ink' : ''}>
          ...
          <div className="flex justify-between gap-4 border-t border-black/10 pt-1 font-semibold">
            <dt>= Restante</dt>
            <dd className={summary.remaining < 0 ? 'text-rose-600' : 'text-emerald-600'}>
      ...
      <button type="button" className="btn-primary w-full py-3 text-base" onClick={openNew}>
        + Añadir gasto
      </button>

      <Card title="Registro semanal">
        {transactions.length === 0 ? (
          <p className="py-6 text-center text-sm text-ink/50">Todavía no hay gastos en este mes.</p>
        ) : (
          <div className="space-y-4">
            {WEEKS.map((week) => {
              ...
              return (
                <div key={week}>
                  <div className="mb-1 flex items-baseline justify-between border-b border-black/5 pb-1">
                    <h3 className="text-xs font-semibold uppercase tracking-wide text-ink/50">Semana {week}</h3>
